"use strict";

(function() {

Yahia.AI.NOISE_FACTOR     = 5;

var pieceEmpty            = 0x00;
var pieceMan              = 0x01;
var pieceKing             = 0x02;
var pieceNo               = 0x80;

var moveflagPromotion     = 0x01000000;

function GenerateCaptureMoves(moves) {
  var color = Yahia.AI.g_toMove ? Yahia.AI.colorWhite : Yahia.AI.colorBlack;
  for (var pos = 0; pos < 256; pos++) {
       if (Yahia.AI.g_board[pos] & color) {
           GenerateCaptureMovesFrom(moves, pos);
       }
  }
}

function GenerateQuietMoves(moves) {
  var color = Yahia.AI.g_toMove ? Yahia.AI.colorWhite : Yahia.AI.colorBlack;
  for (var pos = 0; pos < 256; pos++) {
       if (Yahia.AI.g_board[pos] & color) {
           GenerateQuietMovesFrom(moves, pos);
       }
  }
}

function CheckInvariant(moves) {
  var mx = 0;
  for (var i = 0; i < moves.length; i++) {
      if (mx < moves[i].length) mx = moves[i].length;
  }
  var result = [];
  for (var i = 0; i < moves.length; i++) {
      if (moves[i].length == mx) result.push(moves[i]);
  }
  return result;
}

function CheckPromotion(moves) {
  var result = [];
  for (var i = 0; i < moves.length; i++) {
      if ((moves[i].length > 0) && (moves[i][0] & moveflagPromotion)) result.push(moves[i]);
  }
  return result;
}

Yahia.AI.GenerateAllMoves = function() {
  var moves = [];
  GenerateCaptureMoves(moves);
  moves = CheckInvariant(moves);
  if (moves.length == 0) {
      GenerateQuietMoves(moves);
  }
  return moves;
}

Yahia.AI.GenerateCaptureMoves = function() {
  var moves = [];
  GenerateCaptureMoves(moves);
  moves = CheckInvariant(moves);
  if (moves.length == 0) {
      GenerateQuietMoves(moves);
      moves = CheckPromotion(moves);
  }
  return moves;
}

function GenerateQuietStep(moves, from, to, isMan) {
    var flags = 0;
    if (isMan) {
        var row = to & 0xF0;
        if (!Yahia.AI.g_toMove && (row == 0xA0)) {
            flags = moveflagPromotion;
        }
        if (Yahia.AI.g_toMove && (row == 0x20)) {
            flags = moveflagPromotion;
        }
    }
    moves.push(from | (to << 8) | flags);
}

function GenerateCaptureStep(from, dir, isMan) {
    var enemy = Yahia.AI.g_toMove == Yahia.AI.colorWhite ? Yahia.AI.colorBlack : Yahia.AI.colorWhite;
    var captured = from + dir;
    if (!isMan) {
        while (Yahia.AI.g_board[captured] == pieceEmpty) {
            captured += dir;
        }
    }
    if ((Yahia.AI.g_board[captured] & enemy) == 0) return 0;
    var to = captured + dir;
    if (Yahia.AI.g_board[to] != pieceEmpty) return 0;
    var flags = 0;
    if (isMan) {
        var row = to & 0xF0;
        if (!Yahia.AI.g_toMove && (row == 0xA0)) {
            flags = moveflagPromotion;
        }
        if (Yahia.AI.g_toMove && (row == 0x20)) {
            flags = moveflagPromotion;
        }
    }
    return from | (to << 8) | (captured << 16) | flags;
}

function GenerateCaptureMovesFromTree(moves, from, isMan, isProm, stack, restricted) {
    var r = true;
    var dirs = [1, -1, 16, -16];
    if (_.indexOf(Yahia.AI.SPEC_POSITIONS, +from) >= 0) {
        dirs = [1, -1, 16, -16, -17, -15, 15, 17];
    }
    _.each(dirs, function(dir) {
        if (restricted && (restricted == dir)) return;
        var step = GenerateCaptureStep(from, dir, isMan);
        if (step == 0) return;
        var f = false;
        if (step & moveflagPromotion) {
            f = true;
            step &= ~moveflagPromotion;
        }
        var pos = (step >> 8) & 0xFF;
        stack.push(step);
        Yahia.AI.MakeStep(step, 0);
        if (GenerateCaptureMovesFromTree(moves, pos, isMan, f, stack, -dir)) r = false;
        Yahia.AI.UnmakeStep();
        stack.pop();
        if (!isMan) {
            pos += dir;
            while (Yahia.AI.g_board[pos] == pieceEmpty) {
                step &= ~0xFF00;
                step |= pos << 8;
                stack.push(step);
                Yahia.AI.MakeStep(step, 0);
                if (GenerateCaptureMovesFromTree(moves, pos, isMan, f, stack, -dir)) r = false;
                Yahia.AI.UnmakeStep();
                stack.pop();
                pos += dir;
            }
        }
    });
    if (r && (stack.length > 0)) {
        var move = new Array();
        for (var i = 0; i < stack.length; i++) {
             var m = stack[i];
             if (isProm && (i == stack.length - 1)) {
                 m |= moveflagPromotion;
             }
             move.push(m);
        }
        moves.push(move);
    }
    return !r;
}

function GenerateCaptureMovesFrom(moves, from) {
    var piece = Yahia.AI.g_board[from] & Yahia.AI.TYPE_MASK;   
    GenerateCaptureMovesFromTree(moves, from, piece == pieceMan, false, new Array());
}

function GenerateQuietMovesFrom(moves, from) {
    var to; var steps;
    var inc = (Yahia.AI.g_toMove == Yahia.AI.colorWhite) ? -16 : 16;
    var piece = Yahia.AI.g_board[from] & Yahia.AI.TYPE_MASK;

    if (piece == pieceMan) {
        to = +from + inc; 
        if (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, true);
            moves.push(steps);
        }
        if (_.indexOf(Yahia.AI.SPEC_POSITIONS, +from) < 0) return;
        to = +from + inc - 1; 
        if (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, true);
            moves.push(steps);
        }
        to = +from + inc + 1; 
        if (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, true);
            moves.push(steps);
        }
    }

    if (piece == pieceKing) {
        to = from - 1;
        while (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, false);
            moves.push(steps);
            to--;
        }
        to = +from + 1;
        while (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, false);
            moves.push(steps);
            to++;
        }
        to = from - 16;
        while (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, false);
            moves.push(steps);
            to -= 16;
        }
        to = +from + 16;
        while (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, false);
            moves.push(steps);
            to += 16;
        }
        if (_.indexOf(Yahia.AI.SPEC_POSITIONS, +from) < 0) return;

        to = from - 17;
        while (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, false);
            moves.push(steps);
            to -= 17;
        }
        to = +from + 17;
        while (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, false);
            moves.push(steps);
            to += 17;
        }
        to = from - 15;
        while (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, false);
            moves.push(steps);
            to -= 15;
        }
        to = +from + 15;
        while (Yahia.AI.g_board[to] == 0) {
            steps = new Array();
            GenerateQuietStep(steps, from, to, false);
            moves.push(steps);
            to += 15;
        }
    }
}

})();
