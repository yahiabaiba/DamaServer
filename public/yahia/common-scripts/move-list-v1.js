...
Yahia.Model.getMoveList = function(board) {
  board.generate();
...
MoveList.prototype.getStops = function() {
  ...
  if (Yahia.Model.smartFrom || Yahia.Model.smartTo) {
      var positions = [];
      ...
      _.each(_.keys(positions), function(pos) {
            if ((positions[pos] == 1) || 
//              (Yahia.Model.smartTo && (positions[pos] > 1) && (this.level > 0)) ||
                (Yahia.Model.smartTo && (positions[pos] > 1) && this.isUniqDest(+pos))) {
                result.push(+pos);
            }
      }, this);
  }
...
MoveList.prototype.setPosition = function(pos) {
  var result = Yahia.Model.createMove();
  if (_.indexOf(this.getStops(), pos) >= 0) {
      var moves = _.filter(this.moves, function(move) {
          ...
          if (m.length > 0) {
              ...
              if (Yahia.Model.smartFrom && (this.position == null) && (_.indexOf(m[0][0], pos) >= 0)) {
                  // Smart from move
                  m[0][0] = [ pos ];
                  this.copyActions(result, actions, move.mode, move.sound);
                  return true;
              }
              if (Yahia.Model.smartTo && (this.position == null) && (_.indexOf(m[0][1], pos) >= 0)) {
                  // Smart from move
                  m[0][1] = [ pos ];
                  this.copyActions(result, actions, move.mode, move.sound);
                  return true;
...
Yahia.Controller.SelectPiece = function(move, piece, part) {
  _.each(move.actions, function(a) {
...
MoveList.prototype.setPiece = function(piece) {
  ...
  _.each(this.moves, function(move) {
      if (f) return;
      f = Yahia.Controller.SelectPiece(move, piece, this.level);
      moves.push(move);
      if (result.isPass()) {
          _.each(move.actions, function(a) {
