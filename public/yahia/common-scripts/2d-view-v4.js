...
Yahia.View.STEP_CNT     = 3;

Yahia.View.SHIFT_X      = 0;
Yahia.View.SHIFT_Y      = 0;

Yahia.View.STRIKE_ALPHA = 0.5;
Yahia.View.DROPS_ALPHA  = 0.5;
...
Yahia.View.configure = function(view) {}
...
Yahia.View.getView = function() {
  if (_.isUndefined(Yahia.View.view)) {
      Yahia.View.view = new View2D();
  }
  return Yahia.View.view;
}

Yahia.View.inRect = function(view, pos, x, y) {
...
Yahia.View.pointToPositions = function(view, x, y) {
  if (view.stack.length > 0) {
      ...
      for (var i = 0; i < popup.g.length; i++) {
           ...
           if ((x > sx) && (y > sy) &&
               ...
               return [ Yahia.Model.stringToPos(pos.name) ];
           }
      }
      return [];
  } else {
      return _.chain(_.range(view.pos.length))
       .filter(function(pos) {
          return Yahia.View.inRect(view, pos, x, y);
        })
       .value();
  }
}
...
View2D.prototype.pointToPositions = function(x, y) {
  return Yahia.View.pointToPositions(this, x, y);
}

View2D.prototype.defPosition = function(name, x, y, dx, dy) {
  var ix = Yahia.Model.stringToPos(name);
  this.pos[ix] = {
      x: x, dx: dx,
      y: y, dy: dy
  };
...
View2D.prototype.getY = function(ix) {
  var r = this.pos[ix].y;
  if (!_.isUndefined(Yahia.View.deltaY)) {
      r += Yahia.View.deltaY(ix);
  }
  return r;
}
...
View2D.prototype.getPopupPositions = function(ix) {
  ...
  if (ix < this.popups.length) {
      _.each(this.popups[ix].g, function(f) {
           r.push(Yahia.Model.stringToPos(f.name));
      });
  }
  return r;
}
...
View2D.prototype.movePiece = function(move, from, to, piece, phase, steps) {
  ...
  if (_.isUndefined(steps)) { steps = Yahia.View.STEP_CNT; }
  this.queue[phase].push({
      cnt: steps,
      piece: piece,
      from: from,
...
View2D.prototype.markPositions = function(type, positions) {
  if (type == Yahia.View.markType.TARGET) {
      this.target = positions;
  } 
  if (type == Yahia.View.markType.ATTACKING) {
      this.strike = positions;
  }
  if (type == Yahia.View.markType.CURRENT) {
      this.current = positions;
  }
  if (type == Yahia.View.markType.KO) {
      this.ko = positions;
  }
  if (type == Yahia.View.markType.GOAL) {
      this.goal = positions;
  }
  this.invalidate();
}
...
View2D.prototype.showKo = function(ctx) {
   if (!_.isUndefined(this.piece["Ko"]) && (this.ko.length > 0)) {
       ...
       _.each(this.ko, function(pos) {
          ...
          if (!_.isUndefined(Yahia.View.KO_ALPHA)) {
              ctx.save();
              ctx.globalAlpha = Yahia.View.KO_ALPHA;
          }
...
Yahia.View.showPiece = function(view, ctx, frame, pos, piece, model, x, y) {
  var isSaved = false;
  var dx = 0;
  var dy = 0;
  if (_.indexOf(view.strike, pos) >= 0) {
      ctx.save();
      ctx.globalAlpha = Yahia.View.STRIKE_ALPHA;
      isSaved = true;
  }
  if (Yahia.Model.showBlink && (_.indexOf(view.current, pos) >= 0)) {
      dx = blink;
  }
...
Yahia.View.shiftX = function(pos) {
  return Yahia.View.SHIFT_X;
}

Yahia.View.shiftY = function(pos) {
  return Yahia.View.SHIFT_Y;
}
...
var drawMarks = function(ctx, view, list, color) {
   _.each(list, function(p) {
        ...
        if (!_.isUndefined(Yahia.View.MARK_R)) {
            r = Yahia.View.MARK_R;
        }
        ctx.beginPath();
        ctx.fillStyle = color;
        ctx.arc(x + Yahia.View.shiftX(p), y + Yahia.View.shiftY(p), r, 0, 2 * Math.PI);
        ctx.fill();
        ctx.stroke();
   }, view);
}

Yahia.View.showMarks = function(view, ctx) {
...
View2D.prototype.showDrops = function(ctx) {
  if (this.drops.length > 0) {
      ctx.save();
      ctx.globalAlpha = Yahia.View.DROPS_ALPHA;
  ...
  if (this.allResLoaded()) {
      ...
      _.each(this.back, function(b) {
           if (!_.isUndefined(b.t)) {
               var board = Yahia.Controller.app.board;
     ...
     .each(function(s) {
           ...
           if (piece) {
               x += (pos.dx - piece.dx) / 2 | 0;
               y += (pos.dy - piece.dy) / 2 | 0;
               Yahia.View.showPiece(this, ctx, pos, s.pos, piece, s.piece, x, y, s);
           }
      }, this);
      this.showKo(ctx);
      Yahia.View.showMarks(this, ctx);
      this.showDrops(ctx);
      if (!_.isUndefined(Yahia.View.showBoard)) {
           var board = this.controller.getBoard();
           Yahia.View.showBoard(board, ctx);
      }
      this.drawPopups(ctx);
  }
}

Yahia.View.PKM = function(view, positions) {}
