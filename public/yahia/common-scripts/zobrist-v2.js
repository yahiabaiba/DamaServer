...
function ZobristHash() {
   this.mt = new Yahia.AI.MT(0x1badf00d);
   this.lh = []; this.hh = [];
   this.lp = []; this.hp = [];
}

Yahia.Model.getZobristHash = function() {
  if (_.isUndefined(Yahia.Model.zobrist)) {
      Yahia.Model.zobrist = new ZobristHash();
  }
  return Yahia.Model.zobrist;
}
...
Yahia.Model.getPieceType = function(piece) {
  return piece.type;
}

Yahia.Model.zupdate = function(value, piece, pos) {
  var z = Yahia.Model.getZobristHash();
  return z.lpos(value, piece.player, Yahia.Model.getPieceType(piece), pos);
}

Yahia.Model.hupdate = function(value, piece, pos) {
  var z = Yahia.Model.getZobristHash();
  return z.hpos(value, piece.player, Yahia.Model.getPieceType(piece), pos);
}
